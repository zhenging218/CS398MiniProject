	...
	namespace GPUMinimax
	{
		__global__ void white_min_kernel(Minimax::utility_type *v, GPUBitBoard src, int alpha, int beta, int depth, int turns)
		{
			...
			else
			{
				...
				for (int i = 0; i < 4; ++i)
				{
					white_max_kernel << <dim3(1, 1, 1), dim3(32, 1, 1), 0, streams[i] >> >(utility, new_boards[i], alpha, t_beta, depth - 1, turns - 1);
		...
		__global__ void white_max_kernel(Minimax::utility_type *v, GPUBitBoard src, int alpha, int beta, int depth, int turns)
		{
			...
			else
			{
				...
				for (int i = 0; i < 4; ++i)
				{
					white_min_kernel << <dim3(1, 1, 1), dim3(32, 1, 1), 0, streams[i] >> >(utility, new_boards[i], t_alpha, beta, depth - 1, turns - 1);
					cudaStreamWaitEvent(streams[i], stream_events[i], 0);
				}
