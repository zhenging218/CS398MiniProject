	...
	namespace GPUMinimax
	{
		__global__ void white_min_kernel(utility_type *v, GPUBitBoard src, int alpha, int beta, int depth, int turns)
		{
			...
			else
			{
				...
				GPUBitBoard::gen_black_move[gen_black_move_type](1u << tx, new_boards, src);

				for (int i = 0; i < 4; ++i)
				{
					cudaEventCreate(&stream_events[i][0]);
					cudaEventCreate(&stream_events[i][1]);
				}

				for (int i = 0; i < 4; ++i)
				{
					cudaEventRecord(stream_events[i][0], streams[i]);
					white_max_kernel << <dim3(1, 1, 1), dim3(32, 1, 1), streams[i] >> >(utility, new_boards[i], alpha, t_beta, depth - 1, turns - 1);
					cudaEventRecord(stream_events[i][1], streams[i]);
				}

				// synchronize all events
				for (int i = 0; i < 4; ++i)
				{
					cudaEventSynchronize(stream_events[i][1]);
					cudaEventDestroy(stream_events[i][0]);
					cudaEventDestroy(stream_events[i][1]);
				}
		...
		__global__ void white_max_kernel(utility_type *v, GPUBitBoard src, int alpha, int beta, int depth, int turns)
		{
			...
			else
			{
				...
				GPUBitBoard::gen_white_move[gen_white_move_type](1u << tx, new_boards, src);
				...
				{
					cudaEventRecord(stream_events[i][0], streams[i]);
					white_min_kernel << <dim3(1, 1, 1), dim3(32, 1, 1), streams[i] >> >(utility, new_boards[i], alpha, t_beta, depth - 1, turns - 1);
					cudaEventRecord(stream_events[i][1], streams[i]);
				}
